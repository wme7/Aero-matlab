...
pause;    disp('(1a) Unit circle, h=0.05')


fd=inline('sqrt(sum(p.^2,2))-1','p');
[p,t,q,u]=odtmesh2d(fd,@huniform,0.05,[-1,-1;1,1],[]);
...
box=[-1,-1;1,1];
[p,t,q,u]=odtmesh2d(fd,@huniform,0.05,box,[]);
...
box=[-1,-1;1,1];
fix=[-1,-1;-1,1;1,-1;1,1];
[p,t,q,u]=odtmesh2d(fd,@huniform,0.035,box,fix);
...
[p,t,q,u]=odtmesh2d(fd,fh,0.02,box,fix);
...
box=[-1,-1;1,1];
fix=[cos(phi),sin(phi)];
[p,t,q,u]=odtmesh2d(fd,@huniform,0.05,box,[fix;.5*fix*[cos(pi/6),-sin(pi/6);sin(pi/6),cos(pi/6)]],fix);
% % post(p,t,@huniform)
measure(k,1) = min(q);
measure(k,2) = mean(q);
measure(k,3) = u;
...
[p,t,q,u]=odtmesh2d(@fd5,@fh5,0.01,[-1,0;1,1],fix);
...
fix=[fix;0,0;0.9,0];
box=[0,-1;1,1];
[p,t,q,u]=odtmesh2d(@fd8,@fh8,0.0025,box,fix);
