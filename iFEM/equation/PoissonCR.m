function [u,Du,eqn,info] = PoissonCR(node,elem,pde,bdFlag,option)
%% POISSONCR Poisson equation: Crouzeix-Raviart element.
%
% Assembel the matrix Ax=F for the Crouzeix and Raviart nonconforming
...
%
%    u = POISSONCR(node,elem,bdFlag,f,g_D,g_N)  % mixed boundary condition
%   [u,A,F] = PoissonCR(node,elem,bdFlag,f,g_D,g_N) 
%
% Example
%
%    exampleCR
...
if ~isempty(pde.f) 
    ...
    phi = 1-2*lambda;                 % linear bases for CR
	nQuad = size(lambda,1);
    bt = zeros(NT,3);
    for p = 1:nQuad
