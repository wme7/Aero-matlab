function [u,Du,eqn,info] = Poisson3CR(node,elem,pde,bdFlag,option,varargin)
%% POISSON3CR Poisson equation: Crouzeix-Raviart element in 3-D.
%
% Assembel the matrix Ax=b for the Crouzeix and Raviart nonconforming
% linear finite element discritization of Poisson equation and solve it
%          $-\Delta u = f $    in  $\Omega$ 
...
%
%    u = POISSON3CR(node,elem,pde,bdFlag,option,varagin)
%    pde: the load and boundary conditions
%
% Example
%
%   femrate3CR
%
% Modified by Lin Zhong and Ming Wang.
%
...
if isPureNeumann
    patchVolume = accumarray(elem2face(:),repmat(volume,4,1), [NF 1]); 
    uc = sum(u.*patchVolume)/4;
    u = u - uc;   % int u = 0
end

%% Output information
eqn = struct('A',AD,'b',b,'face',face,'freeFace',freeFace,'isPureNeumann',isPureNeumann);
