function [sigma,u] = HodgeLaplacian3E(node,elem,pde,bdFlag,option)
%% HODEGELAPLACIAN3E Hodge Laplacian of edge element
...
Mv = spdiags(Mv,0,N,N);
Me = getmassmat3(elem2edge,volume,Dlambda,'ND1');
...
%% Boundary Conditions
[AD,f,u,sigma,freeDof,isPureNeumann] = getbdHodgeLap3E(f);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function [AD,f,u,sigma,freeDof,isPureNeumann] = getbdHodgeLap3E(f)

    %% Neumann boundary condition
    [fixedNode,Neumann,isBdNode] = findboundary3(elem,bdFlag);    
    % Part u: int_F (curl u cross n, v)
    g = zeros(Ndof,1);
    if isnumeric(pde.gcurlu) && all(pde.gcurlu == 0)   % zero gu
        pde.gcurlu = [];
    end
