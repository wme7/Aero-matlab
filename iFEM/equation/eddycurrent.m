function [u,edge,eqn,info] = eddycurrent(node,elem,pde,bdFlag,option)
%% EDDYCURRENT Eddy current equation: lowest order edge element.
%
% u = EDDYCURRENT(node,elem,pde,bdFlag) produces the lowest order edge
%   element approximation of the electric field of the following eddy
%   current equation.
%
% curl(mu^(-1)curl u) + beta*beta u = J    in \Omega,  
...
% 
% u = EDDYCURRENT(node,elem,pde,bdFlag,option) specifies the solver options.
