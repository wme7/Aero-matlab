function [u,p,edge,A,eqn,info] = StokesMini(node,elem,pde,bdFlag,option)
%% STOKESMINI Stokes equation: MINI elements.
%
%   [u,p] = STOKESMini(node,elem,pde,bdFlag) use continous P1+bubble
...
%
%  Direct discretization of MINI element yields the following system:
%
%             | A  O  B' ||u |    |f|
%             | O  D  Bb'||ub| =  |g|
%             | B  Bb 0  ||p |    |0|
%
%  Eliminating the interior bubble DOFs 'ub' gives us the stablized system
%
%             | A  B' | |u|    |f |
%             |       | | | =  |  |
%             | B -Ap | |p|    |g'|,
%
%  where Ap = Bb*D^{-1}Bb and g' = -Bb*D^{-1}*g.
%
...
N = size(node,1);  NT = size(elem,1); Nu = N; Np = N;
...
A = blkdiag(A,A);
%** Buble function part: (D^{-1} is needed for Ap and G')
%      D = sum_i(\nabla \lambda_i,\nabla \lambda_i)/180
invD = spdiags(180./Dij,0,NT,NT);
invD = blkdiag(invD,invD);
...
for i = 1:3 % loop for p index
    ...
    Dxb = Dxb + sparse(elem(:,i), (1:NT), -1/60*Dphi(:,1,i).*area, N, NT);
    Dyb = Dyb + sparse(elem(:,i), (1:NT), -1/60*Dphi(:,2,i).*area, N, NT);
end
B = -[Dx Dy]; Bb = -[Dxb Dyb] ;
...
if ~isempty(pde.f)
    ...
    phi = lambda(:,1).*lambda(:,2).*lambda(:,3);
    nQuad = size(lambda,1);
...
%% Boundary condition
[AD,BD,f,g,u,p,ufreeDof,pDof] = getbdStokesMini;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% subfunctions getbdStokesMini
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function [AD,BD,f,g,u,p,ufreeDof,pDof] = getbdStokesMini
        %% Boundary condition of Stokes equation: Mini element
    ...
    end % end of function getbdStokesMini
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end