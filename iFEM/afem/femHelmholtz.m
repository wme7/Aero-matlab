...
%% Display error
% ts = zeros(i,3); ts = char(ts);
% display(' #Dof   ||u-u_h||     ||Du-Du_h||   ||DuI-Du_h||  ||uI-u_h||_{max} iter solvetime');
% display([num2str(floor(sqrt(err.N))) ts num2str(err.L2,'%0.5e') ts num2str(err.H1,'%0.5e')...
%          ts num2str(err.uIuhH1,'%0.5e') ts num2str(err.uIuhMax,'%0.5e')     ts ts ts num2str(solver.itStep) ts num2str(solver.time)] );
global k     
ts = zeros(i,3); ts = char(ts);
n = floor(sqrt(err.N)-1);
display(' #n   ||u-u_h|| iter solvetime k*h');
display([num2str(n) ts num2str(err.L2,'%0.5e') ts ...
         num2str(solver.itStep) ts num2str(solver.time,'%0.5e') ts num2str(k./n,'%0.5e')]);     