...
for i = 1:maxIt
    [w,u,eqn,inf] = fourCurl3P2(node,elem,bdFlag,pde,option);
    ...
    errwL2(i) = getL2error3NE2(node,elem,pde.curlcurlu,w);
    erruL2(i) = getL2error3NE2(node,elem,pde.exactu,u);   
    errwwhHcurl(i) = getHcurlerror3NE2(node,elem,pde.curlcurlcurlu,w);
    erruuhHcurl(i) = getHcurlerror3NE2(node,elem,pde.curlu,u); 
    errwIwhL2(i) = sqrt((w-wI)'*eqn.M*(w-wI));
    erruIuhL2(i) = sqrt((u-uI)'*eqn.M*(u-uI)); 
     errwIwhHcurl(i) = sqrt((w-wI)'*eqn.A*(w-wI));
     erruIuhHcurl(i) = sqrt((u-uI)'*eqn.A*(u-uI));      
    N(i) = size(node,1);
    N(i) = N(i)^(1/3);
...
h = 1./N(1:maxIt);
subplot(2,2,1)
showrateh2(h, errwIwhL2(1:maxIt), 2 , 'k-+', '||w_I-w_h||',...
                   h, erruIuhL2(1:maxIt), 2 , 'r-*', '||u_I-u_h||' );
subplot(2,2,2)
showrateh2(h, errwL2(1:maxIt), 2 , 'k-+', '||w-w_h||',...
                   h, erruL2(1:maxIt), 2 , 'r-*', '||u-u_h||' );                                     
subplot(2,2,3)               
showrateh2(h, errwIwhHcurl(1:maxIt), 2 , 'b-+', '||w_I-w_h||_1',...
                   h, erruIuhHcurl(1:maxIt), 2 , 'g-*', '||u_I-u_h||_1' );
subplot(2,2,4)               
showrateh2(h, errwwhHcurl(1:maxIt), 2 , 'b-+', '||w-w_h||_1',...
                   h, erruuhHcurl(1:maxIt), 2 , 'g-*', '||u-u_h||_1' );
