...
tEnd      = 0.15;   % Final Time for computation
...
while t < tEnd
    ...
    ut_1 = ut + dt*AdvecResidual(ut,F,dF,S,Ln,Lp,V,D,invM,flux_type);
    ut_2 = 3/4*ut + 1/4*ut_1 + 1/4*dt*AdvecResidual(ut_1,F,dF,S,Ln,Lp,V,D,invM,flux_type);
    ut_next = 1/3*ut + 2/3*ut_2 + 2/3*dt*AdvecResidual(ut_2,F,dF,S,Ln,Lp,V,D,invM,flux_type);
