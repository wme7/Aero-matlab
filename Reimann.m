...
ch=0;
while(ch==0)
    fprintf('Choose on of the following cases :- \n');
    fprintf('\n \t Case 1: Sods problem: Left Expansion & Right Shock \n');
    ...
    fprintf('\t Case 6: Cavitation \n');
        cas=input('\n Enter a case no. <1-6>: ');
    if cas==1
        %Case 1: Left Expansion and Right Shock
    ...
    elseif cas==5
        % Double Expansion
        fprintf('Case 1:Double Expansion \n');
        rho1=1;
        rho4=2.5;
        u1=-2;
        u4=2;
    ...
    elseif cas==6
        ...
        fprintf('Please enter an appropiate choice \n');
    end %for if-else selection loop.
end %for while loop.

%Calculating flow parameters at initial condition
...
% Secant Method for getting pressure P*
    ...
    %Calculation of flow parameters depending whether shock or and expasion
    %is observed.
...
end %end for cavitation test

%-------------------------------------------------------------------------%
% Print Calculated flow quantities
fprintf('\n Solution of Reimann problem :- \n');
fprintf('P* = %f \n',p23);
fprintf('U* = %f \n',u23);
fprintf('rho2 = %f \n',rho2);
fprintf('rho3 = %f \n',rho3);
...
%Calculation of shock/expasion speeds
if s==1
    cs121=a1*sqrt(1+(gamma+1)/(2*gamma)*(p23-p1)/p1);
    cs12=u1-abs(cs121);
else
    expc121=u1-a1;
    expc122=u23-a2;
