...
void mexFunction(int nlhs_m, mxArray *plhs_m[], int nrhs_m, const mxArray *prhs_m[])
{
	...
	{
		work[i*2]=((float*)mxGetData(prhs_m[16]))[i];
		if (mxIsComplex(prhs_m[16]))
			work[i*2+1]=((float*)mxGetImagData(prhs_m[16]))[i];
		else
			work[i*2+1]=0;
	}
	...
	plhs_m[16] = mxCreateNumericMatrix((max(1,*lwork)),(1),mxSINGLE_CLASS,mxCOMPLEX);
	for (i=0; i<(max(1,*lwork))*(1); i++)
	{
		((float*)mxGetData(plhs_m[16]))[i]=work[2*i];
		if (mxIsComplex(plhs_m[16]))
			((float*)mxGetImagData(plhs_m[16]))[i]=work[2*i+1];
	}
