...
void mexFunction(int nlhs_m, mxArray *plhs_m[], int nrhs_m, const mxArray *prhs_m[])
{
	...
	lrwork=(int *)mxGetPr(plhs_m[9]);

	plhs_m[11]=mxDuplicateArray(prhs_m[11]);
	liwork=(int *)mxGetPr(plhs_m[11]);
...
#ifdef F77_INT
	F77_INT* F77_n = n , F77_lda = lda , F77_lwork = lwork , F77_lrwork = lrwork , F77_iwork = iwork , F77_liwork = liwork , F77_info = info ;
#else
	#define F77_n n 
	#define F77_lda lda 
