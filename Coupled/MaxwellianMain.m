...
%     [uxl,tl,pl] = macroproperties1d(rhol,rhoul,El,nx,nv,theta);
        [uxl,tl,pl] = macroproperties1d(rhol,rhoul,El,nx,nv,gamma);
    
    % Apply DOM
     [tl,rhol,uxl] = apply_DOM(tl,rhol,uxl,nv);
%      
%             [tl,uxl]   =  apply2_DOM(tl,uxl,nv);
    % Compute Ml_next
%      Ml_next = f_equilibrium_1d(rhol,uxl,c,tl,theta) ;
      Ml_next = f_equilibrium_1d(rhol,uxl,c,tl) ;
%  **************************************************** 
...
%      [z,ux,t,p] = macroproperties1d(rho,rhou,E,nx,nv,theta);   
         [ux,t,p] = macroproperties1d(rho,rhou,E,nx,nv,gamma);   
   
     
%  update New equilibrium
%      M=f_equilibrium_1d(z,ux,c,t,theta);   
    M=f_equilibrium_1d(rho,ux,c,t);   
     fr=h.*M; 
...
%  preparing new Ml
     ...
     uxl = (1-h).*ux;
     pl = (1-h).*p;
     [uxl,tl,pl]= macroproperties1d(rhol,rhol.*uxl,pl+0.5*rhol.*uxl.^2,nx,nv,theta);
% update New Ml     
%      Ml=   f_equilibrium_1d(zl,uxl,c,tl,theta) ;
Ml=   f_equilibrium_1d(rhol,uxl,c,tl) ;
   Ml(isnan(Ml)) = 0;
