...
% for solving 
%
% $\frac{d \theta}{dt}=-c \theta -\frac{g}{l} \theta$
%
...
theta2(2) = theta2(1) + h*(-c*theta2(1)-g/l*theta1(1));

for i=2:n-1
    %Leapfrog scheeme
    theta1(i+1) = theta1(i-1) + 2*h*theta2(i);
    theta2(i+1) = theta2(i-1) + 2*h*(-c*theta2(i)-g/l*theta1(i));
end
...
theta22(2) = theta22(1) + h*(-c*theta22(1)-g/l*theta11(1));

for i=2:n-1
    %Adams-Bashforth scheeme
    theta11(i+1) = theta11(i) + 3/2*h*theta22(i)...
        - h/2*theta22(i-1);
    theta22(i+1) = theta22(i) + 3/2*h*(-c*theta22(i)-g/l*theta11(i))... 
        - h/2*(-c*theta22(i-1)-g/l*theta11(i-1));
end
%% Exact solution
w  = sqrt(g/l);
c2 = theta1(1)/(1-(-c+sqrt(c^2-4*w^2)/(-c-sqrt(c^2-4*w^2))));
c1 = theta1(1)-c2;
theta = c1*exp(1/2*(-c-sqrt(c^2-4*w^2)).*t)...
    +c2*exp(1/2*(-c+sqrt(c^2-4*w^2)).*t);
