...
% right part of Figs. 5.11, 5.12, 5.14, 5.15
...
nbeta = 2;	% Parameter in exact solution: beta  = nbeta*pi
	% nalpha, nbeta MUST BE EVEN FOR PERIODICITY!
...
if central == 1	  		% Central scheme
  A(1,1) = 2*d;  A(1,2) = sigma/2 - d;
  for j = 2:J-1
    A(j,j-1) = - sigma/2 - d;    A(j,j+1) =  sigma/2 - d;
    A(j,j)   = - A(j,j-1) - A(j,j+1);
  end  
  j = J;  A(j,j-1) = - sigma/2 - d;  A(j,j) = 2*d;
else				% Upwind scheme
  A(1,1) = sigma + 2*d;  A(1,2) =  - d;
  for j = 2:J-1
    A(j,j-1) = -sigma - d;    A(j,j+1) = - d;
    A(j,j)   = - A(j,j-1) - A(j,j+1);
  end
  j = J;  A(j,j-1) = -sigma - d;  A(j,j)   =  sigma + 2*d;
end
if central == 1
  A(1,J) = -sigma/2 - d;  A(J,1) =  sigma/2 - d;
else
  A(1,J) = -sigma - d;  A(J,1) = - d;
end

E = eye(size(A)) + omega*A;		% System to be solved is 
C = eye(size(A)) + (omega-1)*A;		% E*ynew = C*yold + rhs
...
axis([0 1 -1.5 2.5]); plot (x,ynew,'o');
if central == 1
  s1 = ['Central omega scheme'];
else
  s1 = ['Upwind omega scheme'];
end
  s1 = [s1, ', periodic b.c.'];
title(s1,'fontsize',18);
if hom == 1
  s2 = ['Homogeneous differential equation']
else
  s2 = ['Inhomogeneous differential equation']
end
